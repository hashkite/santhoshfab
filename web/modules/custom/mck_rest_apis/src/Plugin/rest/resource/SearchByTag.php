 ...
 * @RestResource(
 *   id = "search_by_tag",
 *   label = @Translation("Search By tag"),
 *   serialization_class = "",
 *   uri_paths = {
 *     "canonical" = "/search-by-tag/{view_id}/{display_id}/{tag_id}/{nid}"
 *   }
 * )
 */
class SearchByTag extends ResourceBase {
   ...
   *   Node output.
   */
  public function get(string $view_id, string $display_id, string $tag_id, $nid, Request $request): CacheableJsonResponse {

    $context = new RenderContext();
    /* @var \Drupal\Core\Cache\CacheableDependencyInterface $result */
    $result = \Drupal::service('renderer')->executeInRenderContext($context, function() use ($view_id, $display_id, $tag_id, $nid) {
      return $this->mckRestManager->getViewValues($view_id, $display_id, NULL, [$tag_id, $nid]);
    });
