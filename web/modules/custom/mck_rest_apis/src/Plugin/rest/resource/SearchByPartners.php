 ...
 *
 * @RestResource(
 *   id = "search_by_partners",
 *   label = @Translation("Search By partners"),
 *   serialization_class = "",
 *   uri_paths = {
 *     "canonical" = "/search-by-partners/{view_id}/{display_id}/{nid}"
 *   }
 * )
 */
class SearchByPartners extends ResourceBase {
   ...
   *   Node output.
   */
  public function get(string $view_id, string $display_id, $nid, Request $request): CacheableJsonResponse {

    $context = new RenderContext();
    /* @var \Drupal\Core\Cache\CacheableDependencyInterface $result */
    $result = \Drupal::service('renderer')->executeInRenderContext($context, function() use ($view_id, $display_id, $nid) {
      return $this->mckRestManager->getViewValues($view_id, $display_id, NULL, [$nid]);
    });
