import classNames from 'classnames';
import { useCallback, useEffect, useState } from 'react';
import { GoogleReCaptcha } from 'react-google-recaptcha-v3';
import { useLocation } from 'react-router-dom';
import { useSubmitForm, useToken } from '../../../shared/api';
import { Loading } from '../../../shared/ui';
import FieldSwitcher from '../../elements/fields';
import SubmissionMessage from './submissionMessage';
import './webform.scss';

const Webform = ({ data, dependentFields, submissionMessage }) => {
  const { fields, webform_id, newsletter_id, action } = data || {};
  const [submittedMessage, setSubmittedMessage] = useState('');
  const [googleReCaptcha, setGoogleReCaptcha] = useState(null);
  const [isSubmitting, setIsSubmitting] = useState(false); // Track submission state
  const [isRequired, setIsRequired] = useState(null);
  const { pathname } = useLocation();

  const initialFormData = fields?.reduce(
    (formData, field) => {
      if (field.type !== 'webform_actions') {
        formData[field.name] = '';
      }
      return formData;
    },
    webform_id
      ? {
          webform_id,
        }
      : newsletter_id
      ? {
          newsletter_id,
        }
      : {}
  );

  const [formData, setFormData] = useState(initialFormData);
  const { data: tokenData } = useToken();
  const { mutate } = useSubmitForm(
    setSubmittedMessage,
    setIsSubmitting,
    action
  );

  const handleInputChange = event => {
    const { name, value } = event.target;
    setFormData({
      ...formData,
      [name]: value,
    });
  };

  useEffect(() => {
    if (dependentFields) {
      setIsRequired(true);
      // Check if any field has been filled, and update the required status accordingly
      Object.keys(formData).forEach(fieldName => {
        if (fieldName !== 'webform_id') {
          if (formData[fieldName]) {
            setIsRequired(false);
          }
        }
      });
    }
  }, [formData, dependentFields]);

  const onVerify = useCallback(token => {
    setGoogleReCaptcha(token);
  }, []);

  const handleSubmit = async (event, data) => {
    if (event) event.preventDefault();
    if (!googleReCaptcha) {
      alert('Please verify you are not a robot');
      return;
    }
    setIsSubmitting(true); // Set isSubmitting to true during form submission
    const token = tokenData?.data;
    const formDataNew = data ? { ...initialFormData, ...data } : formData;

    try {
      setIsSubmitting(true); // Set isSubmitting to true during form submission
      await mutate({ formData: formDataNew, token });
      setTimeout(() => {
        if (
          webform_id === 'sub_services_form' &&
          pathname === '/managed-teams'
        ) {
          window.location.href = '/hire-managed';
        }
      }, 300);
      if (webform_id === 'sub_service' && pathname.includes('/services')) {
        window.location.href = '/hire-services';
      }
    } catch (error) {
      setSubmittedMessage('error');
      console.error('Form submission failed:', error);
    }
  };

  const triggerSubmit = (name, value) => {
    if (webform_id === 'sub_services_form' && pathname === '/managed-teams') {
      handleSubmit(false, { [name]: value });
    }
  };

  if (isSubmitting) {
    return (
      <div className="container webform__loading pt-xl pb-xl">
        <Loading />
      </div>
    );
  }

  if (submittedMessage) {
    return (
      <div className="container submittedMessage">
        <SubmissionMessage
          status={submittedMessage}
          message={submissionMessage}
        />
      </div>
    );
  }

  return (
    <form
      onSubmit={handleSubmit}
      className={classNames(
        'webform__form',
        webform_id,
        webform_id === 'sub_service' && pathname === '/services' && '--services'
      )}
    >
      {fields?.map((field, i) => {
        return (
          <fieldset key={i} className={`fieldset-${field.type}`}>
            <FieldSwitcher
              isRequired={isRequired}
              onChange={handleInputChange}
              triggerSubmit={triggerSubmit}
              {...field}
            />
          </fieldset>
        );
      })}
      <GoogleReCaptcha onVerify={onVerify} />
      {googleReCaptcha && (
        <div className="custom-captcha">
          <div className="text">
            <p>protected by reCAPTCHA</p>
            <div className="links">
              <a
                href="https://policies.google.com/privacy"
                target="_blank"
                rel="noreferrer"
              >
                Privacy
              </a>
              <a
                href="https://policies.google.com/terms"
                target="_blank"
                rel="noreferrer"
              >
                Terms
              </a>
            </div>
          </div>
          <svg
            viewBox="0 -0.04 600 598.59"
            xmlns="http://www.w3.org/2000/svg"
            width="2500"
            height="2494"
            className="recaptcha-logo"
          >
            <path
              d="M600 298.83c-.01-4.3-.11-8.58-.31-12.84V43.06l-67.12 67.16C477.64 42.93 394.06-.04 300.46-.04c-97.42 0-183.96 46.52-238.67 118.57l110.02 111.25a145.91 145.91 0 0 1 44.57-50.04c19.21-14.99 46.43-27.26 84.07-27.26 4.55 0 8.06.54 10.64 1.54 46.65 3.68 87.08 29.44 110.89 66.82l-77.88 77.93c98.64-.39 210.08-.62 255.89.05"
              fill="#1c3aa9"
            />
            <path
              d="M298.71-.03c-4.31.02-8.58.12-12.84.31H43.07l67.13 67.16C42.95 122.41 0 206.02 0 299.68c0 97.47 46.5 184.07 118.51 238.8L229.69 428.4a145.737 145.737 0 0 1-50-44.59c-14.99-19.22-27.25-46.45-27.25-84.12 0-4.55.53-8.07 1.53-10.65 3.69-46.67 29.43-87.12 66.79-110.94l77.88 77.92c-.39-98.7-.62-210.2.05-256.04"
              fill="#4285f4"
            />
            <path
              d="M.01 299.68c.02 4.3.12 8.58.31 12.84v242.93l67.12-67.16c54.94 67.29 138.51 110.26 232.12 110.26 97.41 0 183.95-46.52 238.66-118.57L428.2 368.73c-10.78 19.96-26.1 37.09-44.57 50.04-19.21 14.99-46.42 27.26-84.07 27.26-4.55 0-8.06-.54-10.64-1.54-46.64-3.68-87.08-29.44-110.88-66.82l77.88-77.93c-98.65.39-210.08.62-255.9-.05"
              fill="#ababab"
            />
          </svg>
        </div>
      )}
    </form>
  );
};

export default Webform;
